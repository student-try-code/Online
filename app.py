...
@st.cache_data(ttl=0)
def load_google_sheets(spreadsheet_id):
...
def save_to_google_sheets(spreadsheet_id, all_sheets):
    client = connect_to_google_sheets()
    spreadsheet = client.open_by_key(spreadsheet_id)
    ...
    for sheet_name, df in all_sheets.items():
        ...
        # Chuyển DataFrame thành danh sách các giá trị
        values = [df.columns.values.tolist()] + df.values.tolist()

        # Xóa toàn bộ dữ liệu hiện có và ghi dữ liệu mới
        worksheet.clear()
...
# Sử dụng spreadsheet_id từ secrets.toml
spreadsheet_id = "1Gd4ZEMhKvzxpUhLL4C8lmSzI2RAeFgr8QPBu0efn6IA"  # Lấy từ URL Google Sheets

try:
    all_sheets = load_google_sheets(spreadsheet_id)
except Exception as e:
    st.error(f"❌ Lỗi khi kết nối với Google Sheets: {str(e)}")
    st.stop()
    ...
    if st.session_state.get(row_id + '_edit_original'):
        ...
        if st.button(f"💾 Lưu Original ({oid})"):
            ...
            if not idx.empty:
                ...
                save_to_google_sheets(spreadsheet_id, all_sheets)
